...
For Codex, the repo also provides `.codex/config.toml` agent defaults so Codex can spawn native subagents against these project-scoped agent definitions. The plugin-facing skill surface is generated into `.codex/skills/` by `scripts/build-codex-skills.sh`, and `.codex-plugin/plugin.json` points Codex at that generated directory instead of the Claude-native source files. In shell-driven workflows, `OCTOPUS_CODEX_NATIVE_SUBAGENTS=auto` stays conservative: native host sessions defer to Codex's own runtime behavior, while external CLI dispatch falls back to the existing bridge path.
